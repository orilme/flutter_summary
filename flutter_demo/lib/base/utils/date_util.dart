...
class DateUtil {
  ...
  /// [format]格式
  /// [locale]时区
  static String getDateFormatStrByDateTime(DateTime dateTime, {String format = 'yyyy-MM-dd HH:mm:ss', String locale = _DATE_LOCALE}) {
    intl.DateFormat dateFormat = intl.DateFormat(format, locale);
  ...
  /// [format] 格式
  /// [locale] 时区
  static DateTime? getDateByTimeStr(String time, {String format = 'yyyy-MM-dd HH:mm:ss', String locale = _DATE_LOCALE}) {
  ...
  /// [currentTime] 当前时间
  /// [intervalType] 间隔类型
  /// return 数组  index-0 起时间 index-止时间
...
///将时长转换成时分秒
class TimeUtil {
  static String formatSecondToStr(int second) {
    String str;
    if (second == null) {
      str = '';
    } else {
      ...
      int sec = (second - hour * 60 * 60 - min * 60) % 60;
      str = hour == 0 ? '' : '$hour小时';
      str += min == 0 ? '' : '$min分';
      str += '$sec秒';
    }
