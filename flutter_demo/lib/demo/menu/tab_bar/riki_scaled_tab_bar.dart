...
/// A material design [RikiScaledTabBar] tab.
///
...
/// See also:
///
///  * [RikiScaledTabBar], which displays a row of tabs.
///  * [ListenableTabBarView], which displays a widget for the currently selected tab.
///  * [TabController], which coordinates tab selection between a [RikiScaledTabBar] and a [ListenableTabBarView].
///  * <https://material.io/design/components/tabs.html>
class RikiScaledTab extends StatelessWidget {
  /// Creates a material design [RikiScaledTabBar] tab.
  ///
  ...
  Widget build(BuildContext context) {
    ...
    Widget label;
    if (icon == null) {
      height = this.height ?? _kTabHeight;
...
class _TabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle textStyle = selected
        ? _igoreFontSizeLerp(defaultStyle, defaultUnselectedStyle, animation.value)!
        : _igoreFontSizeLerp(defaultUnselectedStyle, defaultStyle, animation.value)!;
    ...
    if (selected) {
      _scale = lerpDouble(defaultStyle.fontSize! / defaultUnselectedStyle.fontSize!, 1, animation.value)!;
    } else {
      _scale = lerpDouble(1, defaultStyle.fontSize! / defaultUnselectedStyle.fontSize!, animation.value)!;
    }

    return DefaultTextStyle(
      style: textStyle.copyWith(color: color, fontSize: defaultUnselectedStyle.fontSize!),
  ...
  ///copy form TextStyle.lerp,删除了fontSize 的变动
  TextStyle? _igoreFontSizeLerp(TextStyle? a, TextStyle? b, double? t) {
    assert(t != null);
...
class _TabBarScrollPosition extends ScrollPositionWithSingleContext {
  ...
  final _RikiScaledTabBarState tabBar;
...
/// conjunction with a [ListenableTabBarView].
///
...
/// [ListenableTabBarView.children] list.
///
...
/// See also:
///
///  * [ListenableTabBarView], which displays page views that correspond to each tab.
class RikiScaledTabBar extends StatefulWidget implements PreferredSizeWidget {
  factory RikiScaledTabBar.normal({
    Key? key,
    ...
    double indicatorWidth = 16,
    double labelPadding = 16,
  }) {
    return RikiScaledTabBar(
      controller: controller,
      isScrollable: isScrollable,
      indicator: RikiUnderlineTabIndicator(indicatorWidth: indicatorWidth, borderSide: BorderSide(width: 2.w)),
      labelStyle: labelStyle ?? TextStyle(fontSize: 14.5.sp, fontWeight: FontWeight.w500),
      labelColor: labelColor ?? Color(0xff333333),
      unselectedLabelStyle: unselectedLabelStyle ?? TextStyle(fontSize: 14.5.sp, fontWeight: FontWeight.w400),
      unselectedLabelColor: unselectedLabelColor ?? Color(0xff999999),
      tabs: textList?.map((text) => RikiScaledTab(text: text, height: tabHeight)).toList() ?? [RikiEmpty.ui],
      onTap: onTap,
      tabHeight: tabHeight,
  ...
  /// Typically a list of two or more [RikiScaledTab] widgets.
  ///
  /// The length of this list must match the [controller]'s [TabController.length]
  /// and the length of the [ListenableTabBarView.children] list.
  ...
  /// and the entire [RikiScaledTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// the indicator with the tab's text for [RikiScaledTab] widgets and all but the
  /// shortest [RikiScaledTab.text] values.
  ///
  ...
  /// An optional callback that's called when the [RikiScaledTabBar] is tapped.
  ///
  ...
  /// How the [RikiScaledTabBar]'s scroll view should respond to user input.
  ///
  ...
  Size get preferredSize {
    for (final Widget item in tabs) {
      if (item is RikiScaledTab) {
        final RikiScaledTab tab = item;
        if ((tab.text != null || tab.child != null) && tab.icon != null) return Size.fromHeight(_kTextAndIconTabHeight + indicatorWeight);
      }
    }
    return Size.fromHeight((tabHeight ?? _kTabHeight) + indicatorWeight);
  }

  @override
  _RikiScaledTabBarState createState() => _RikiScaledTabBarState();
}

class _RikiScaledTabBarState extends State<RikiScaledTabBar> {
  ...
  @override
  void didUpdateWidget(RikiScaledTabBar oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller!.length == 0) {
      return Container(
        height: (widget.tabHeight ?? _kTabHeight) + widget.indicatorWeight,
      );
    }
